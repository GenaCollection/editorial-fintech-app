...
export default function EarlyPage({ language }) {
  const { loanState, monthlyPayment, totalInterest, schedule } = useLoan()
  const symbol = '֏'

  const scenarios = useMemo(() => {
    return [50000, 100000, 200000].map(extraAmt => {
      const extraPayments = Array.from({ length: loanState.term }, (_, i) => ({ month: i+1, amount: extraAmt }))
      const { schedule: sc } = generateAmortization(loanState.amount, loanState.rate, loanState.term, extraPayments)
      const interest = sc.reduce((s,r) => s + r.interest, 0)
      const saved = Math.max(0, totalInterest - interest)
      return { extraAmt, months: sc.length, monthsSaved: Math.max(0, schedule.length - sc.length), saved }
    })
  ...
  const l = (key, section = 'early') => {
    const m = { title:{AM:'Վаղажамкет марман телекатвуtyун',RU:'Информация о досрочном погашении',EN:'Early Repayment Intelligence'}, desc:{AM:'Հасkacеk инчпес вахжамкет мардумнерě',RU:'Узнайте, как каждый доп. платёж ускоряет погашение.',EN:'Understand how every extra payment accelerates your loan maturity.'}, extraMonthly:{AM:'Лрацуциш амсакан',RU:'Доп. ежемесячно',EN:'Extra monthly'}, termReduction:{AM:'Жамкети кркатум',RU:'Сокращение срока',EN:'Term reduction'}, interestSaved:{AM:'Хнаяк токос',RU:'Экономия на %',EN:'Interest saved'}, months:{AM:'амис',RU:'мес.',EN:'mo.'} }
    return m[key]?.[language] || m[key]?.EN || key
  }

  return (
    <div className="flex pt-20 min-h-screen">
      <Sidebar language={language} />
      <main className="flex-1 px-6 lg:px-10 py-10 max-w-7xl mx-auto">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-surface-container-low dark:bg-slate-800">
                <tr>
                  {[l('extraMonthly'),'New Term',l('termReduction'),l('interestSaved')].map((h,i) => (
                    <th key={h} className={`px-6 py-4 text-[10px] font-bold text-on-surface-variant uppercase tracking-widest ${i>0?'text-right':'text-left'}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {scenarios.map((sc,i) => (
                  <tr key={i} className="border-t border-slate-50 dark:border-slate-800 hover:bg-primary-fixed/10 dark:hover:bg-blue-900/10 transition-colors">
                    <td className="px-6 py-4 font-headline font-bold text-on-surface dark:text-slate-100">+{symbol}{sc.extraAmt.toLocaleString()}</td>
                    <td className="px-6 py-4 text-right text-on-surface dark:text-slate-300">{sc.months} {l('months')}</td>
                    <td className="px-6 py-4 text-right">
                      <span className="bg-primary/10 dark:bg-blue-900/30 text-primary dark:text-blue-400 font-bold px-2 py-1 rounded-lg text-xs">-{sc.monthsSaved} {l('months')}</span>
                    </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="bg-secondary-container dark:bg-green-900/30 p-8 rounded-2xl flex flex-col justify-between h-52">
            <span className="material-symbols-outlined text-secondary dark:text-green-400 text-4xl">verified</span>
            <div>
              <p className="text-on-secondary-container dark:text-green-100 font-black text-xl">Lowest APR Guarantee</p>
              <p className="text-on-secondary-container/70 text-sm mt-1">Institutional pricing applied.</p>
            </div>
          </div>
          <div className="bg-surface-container-highest dark:bg-slate-800 p-8 rounded-2xl flex flex-col justify-between h-52">
            ...
            <div>
              ...
              <p className="text-on-surface dark:text-slate-100 font-bold">Talk to a Loan Specialist</p>
              <button className="mt-2 text-xs font-bold text-primary dark:text-blue-400 hover:underline">Contact us →</button>
            </div>
          </div>
          <div className="bg-primary-fixed dark:bg-blue-950 p-8 rounded-2xl flex flex-col justify-between h-52">
            <span className="material-symbols-outlined text-primary dark:text-blue-300 text-4xl">trending_up</span>
            <div>
              <p className="text-primary dark:text-blue-200 font-black text-xl">Market Analytics</p>
              <p className="text-on-primary-fixed-variant dark:text-blue-400 text-sm mt-1">Real-time interest rate tracking.</p>
            </div>
          </div>
        </div>
      </main>
