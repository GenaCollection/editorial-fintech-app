...
export default function CalculatorPage({ language }) {
  ...
  const set = (key) => (e) => {
    ...
    setLoanState(prev => ({ ...prev, [key]: Math.max(min, Math.min(max, val || min)) }))
  }

  const slide = (key) => (e) =>
    setLoanState(prev => ({ ...prev, [key]: Number(e.target.value) }))
  ...
  return (
    <main className="flex-1 px-6 md:px-12 pt-28 pb-16 max-w-7xl mx-auto w-full">
      <header className="mb-10">
        <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight mb-2 font-headline">
          {t(language, 'calculator', 'title')}
        </h1>
        <p className="text-xl text-slate-500 dark:text-slate-400">
          {t(language, 'calculator', 'desc')}
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-5 space-y-5">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-7">
              {t(language, 'calculator', 'param')}
            </p>

            {[
              { key: 'amount', label: t(language,'calculator','loanAmt'), min: 100000, max: 100000000, step: 100000, display: amount.toLocaleString(), suffix: SYMBOL },
              { key: 'rate',   label: t(language,'calculator','intRate'), min: 0.1,   max: 100,       step: 0.1,    display: rate,                    suffix: '%' },
                ...
                <div className="flex justify-between items-end mb-3">
                  ...
                  <div className="flex items-baseline gap-1">
                    <input
                      ...
                      value={loanState[key]}
                      onChange={set(key)}
                      className="w-32 text-right text-2xl font-headline font-bold text-slate-900 dark:text-white bg-transparent border-none outline-none p-0"
                    />
            ...
            <div>
              <div className="flex justify-between items-end mb-3">
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">{t(language,'calculator','term')}</span>
                <div className="flex items-baseline gap-1">
                  <input
                    type="number" min={1} max={360} value={term}
                    onChange={set('term')}
                    className="w-16 text-right text-2xl font-headline font-bold text-slate-900 dark:text-white bg-transparent border-none outline-none p-0"
                  />
                  <span className="text-slate-400 text-xs">{t(language,'calculator','months')}</span>
                </div>
              </div>
              <input
                type="range" min={1} max={360} value={term}
                onChange={slide('term')}
              ...
              <div className="grid grid-cols-5 gap-2">
                {[6,12,24,36,60].map(v => (
                  <button
                    key={v}
                    onClick={() => setLoanState(p => ({ ...p, term: v }))}
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800">
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-4">Loan Breakdown</p>
            <div className="flex rounded-full overflow-hidden h-3 mb-3">
              <div className="bg-blue-700 transition-all duration-500" style={{ width: principalPct + '%' }} />
              <div className="bg-emerald-500 transition-all duration-500" style={{ width: (100 - principalPct) + '%' }} />
            </div>
        ...
        <section className="lg:col-span-7 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="bg-slate-100 dark:bg-slate-800 p-8 rounded-2xl">
              <p className="text-xs uppercase tracking-widest text-slate-400 mb-2 font-bold">
                {t(language,'calculator','totalInt')}
              </p>
              <h2 className="text-3xl font-extrabold font-headline text-slate-900 dark:text-white">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <span className="font-headline font-bold text-slate-900 dark:text-slate-100">Schedule Preview</span>
              ...
              <tbody>
                {schedule.slice(0,3).map(row => (
                  <tr key={row.month} className="border-t border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-5 py-3 font-bold text-blue-700 dark:text-blue-400">{row.month}</td>
                    <td className="px-5 py-3 text-slate-400">{row.label}</td>
                    <td className="px-5 py-3 text-right font-mono text-slate-800 dark:text-slate-200">{SYMBOL}{Math.round(row.payment).toLocaleString()}</td>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              ...
              className="flex-1 bg-blue-700 text-white py-4 rounded-xl font-headline font-bold hover:bg-blue-800 active:scale-95 transition-all flex items-center justify-center gap-3"
            >
            ...
            <button
              ...
              className="sm:w-44 bg-slate-100 dark:bg-slate-800 text-blue-700 dark:text-blue-400 py-4 rounded-xl font-headline font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors flex items-center justify-center gap-2"
            >
